...
psql -v ON_ERROR_STOP=1 --username "${POSTGRES_USER}" -d "${HUNTFLOW_DATABASE_DB}" <<-EOSQL
  CREATE SCHEMA IF NOT EXISTS tmp_data;
  GRANT ALL ON SCHEMA public TO ${HUNTFLOW_DATABASE_USERNAME};
  GRANT ALL ON SCHEMA tmp_data TO ${HUNTFLOW_DATABASE_USERNAME};
EOSQL

echo "Init keycloak db"
psql -v ON_ERROR_STOP=1 --username "${POSTGRES_USER}" <<-EOSQL
  CREATE USER ${KEYCLOAK_DB_USERNAME} WITH PASSWORD '${KEYCLOAK_DB_PASSWORD}';
  CREATE DATABASE keycloak;
  GRANT ALL PRIVILEGES ON DATABASE keycloak TO ${KEYCLOAK_DB_USERNAME};
EOSQL
