...
class AllVacancies(Base):
    ...
    position = Column(
        String(100), nullable=False, comment="The name of the vacancy (occupation)"
    )
    company = Column(
        String(100), comment="Department (ignored if the DEPARTMENTS are enabled)"
    )
    money = Column(String(100), comment="Salary")
    priority = Column(
        Integer, comment="The priority of a vacancy (0 for usual or 1 for high"
    )
    ...
    additional_fields_list = Column(
        ARRAY(String(50)), comment="List of additional field names"
    )
    ...
    account_vacancy_status_group = Column(
        Integer, comment="Recruitment status group ID"
    )
    flg_close_recruiter = Column(
        Boolean, comment="True if the vacancy was filled by a recruit"
    )
