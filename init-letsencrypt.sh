...
domains=("${DOMAIN_URL}")
echo $domain
email="${DOMAIN_EMAIL}" # Adding a valid address is strongly recommended
...
docker compose -f ./nginx-compose.yml up nginx -d  && docker compose -f ./nginx-compose.yml restart nginx
...
for domain in ${!domains[*]}; do
  ...
  if [ -e "$data_path/conf/live/$domain_name/cert.pem" ]; then
    ...
    docker compose -f nginx-compose.yml run --rm --entrypoint "certbot certonly --webroot -w /var/www/certbot --cert-name $domain_name $domain_args \
